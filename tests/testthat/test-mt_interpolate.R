test_that("basic interpolation", {
  d <- data.frame(x = c(1:5), y = c(1:5), timestamp = 1:5, track = gl(1, 5), timestamp2 = c(0, 1, 3, 6, 10)) |>
    st_as_sf(coords = 1:2) |>
...
test_that("time_gap", {
  t <- c(1, 3, 5, 6, 7)
  m <- mt_as_move2(data.frame(x = t, y = t, t = t, id = "a"), coords = 1:2, time_column = "t", track_id_column = "id")
  m$geometry[c(2, 4)] <- sf::st_point()
...
test_that("time_gap with posixct", {
  ...
  expect_identical(which(st_is_empty(mt_interpolate(m, max_time_lag = set_units(5, "days")))), integer(0))
  expect_identical(which(st_is_empty(mt_interpolate(m, max_time_lag = set_units(3, "days")))), 2L)
  expect_identical(which(st_is_empty(mt_interpolate(m, max_time_lag = as.difftime(48, units = "hours")))), 2L)
  expect_identical(which(st_is_empty(mt_interpolate(m, max_time_lag = set_units(36, "hour")))), c(2L, 4L))
})
test_that("error different time", {
  m <- mt_sim_brownian_motion(t = c(1:5, 5.5))
  tt <- Sys.time()
  mt <- mt_sim_brownian_motion(t = (as.Date(tt) + -1:4))
