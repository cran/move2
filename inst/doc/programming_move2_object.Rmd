...
- `as.data.frame()`: creates a `data.frame` including all information contained
  in the `move2`, note that for most purposes the `move2` already functions like
  a `data.frame`

- `data.table::as.data.table()`: creates a `data.table` including all
  information contained in the `move2`

- `to_move()`: converts to a object of class `Move`/`MoveStack`

## Transforming other classes to a `move2` object

- `mt_as_move2()`: creates a `move2` object from a `sf`, `data.frame` or
  `Move`/`MoveStack` object
